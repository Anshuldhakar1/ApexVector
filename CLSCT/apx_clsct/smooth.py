...
def fit_bezier_curve(points: np.ndarray, num_control_points: int = 4) -> np.ndarray:
    ...
    # Build basis matrix
    A = np.array(
        [bernstein(i, num_control_points - 1, t) for i in range(num_control_points)]
    ).T
...
def smooth_contour_catmull_rom(contour: Contour, num_points: int = None) -> Contour:
    ...
    for i in range(1, len(closed) - 2):
        segment = catmull_rom_segment(
            closed[i - 1], closed[i], closed[i + 1], closed[i + 2], points_per_segment
        )
...
def smart_smooth_contour(
    contour: Contour,
    long_curve_threshold: int = 100,
    corner_angle_threshold: float = 45.0,
) -> Contour:
    """Intelligently smooth contour while preserving sharp corners.

    Only smooths long curves (> threshold points) and preserves
    corners where angle changes exceed threshold.

    Args:
        contour: Array of (x, y) points
        long_curve_threshold: Minimum contour length to apply smoothing
        corner_angle_threshold: Angle threshold in degrees to detect corners

    Returns:
        Selectively smoothed contour
    """
    if len(contour) < long_curve_threshold:
        return contour
    ...
    for i in range(n):
        ...
        # Sharp corner: angle is small (< threshold) or reflex (> 180 - threshold)
        if angle < corner_angle_threshold or angle > (180 - corner_angle_threshold):
            corners.append(i)

    if len(corners) == 0:
        # No sharp corners, apply gentle smoothing to whole contour
        return gaussian_smooth_contour(contour, sigma=0.8)

    # Process segments between corners
    result = []
    ...
    for i in range(len(corners)):
        ...
        if end > start:
            ...
            # Wrap around
            segment = np.vstack([contour[start:], contour[: end + 1]])

        if len(segment) > 10:
            # Smooth long segments
            segment = gaussian_smooth_contour(segment, sigma=0.5)

        if i == 0:
            result.extend(segment)
        else:
            # Avoid duplicating corner points
